...
async function seed() {
    ...
    // Clear existing
    await Promise.all([
        models_1.Crop.deleteMany({}),
        models_1.Vehicle.deleteMany({}),
        models_1.Mandi.deleteMany({}),
        models_1.MarketPrice.deleteMany({}),
    ]);
    console.log("🗑️  Cleared existing data");
    // Insert crops
    const crops = await models_1.Crop.insertMany(CROPS_DATA);
    console.log(`✅ Inserted ${crops.length} crops`);
    // Insert vehicles
    const vehicles = await models_1.Vehicle.insertMany(VEHICLES_DATA);
    ...
    const mandis = await models_1.Mandi.insertMany(mandisToInsert);
    ...
    await models_1.MarketPrice.insertMany(priceRecords);
