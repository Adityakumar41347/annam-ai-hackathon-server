...
async function resolveVehicle(id) {
    if (mongoose_1.default.isValidObjectId(id)) {
        return models_1.Vehicle.findById(id).lean();
    }
    // Fall back to looking up by value slug e.g. "truck_lcv"
    return models_1.Vehicle.findOne({ value: id }).lean();
}
async function resolveCrop(id) {
    if (mongoose_1.default.isValidObjectId(id)) {
        return models_1.Crop.findById(id).lean();
    }
    // Fall back to looking up by value slug e.g. "onion"
    return models_1.Crop.findOne({ value: id }).lean();
}
...
router.post("/", async (req, res) => {
    try {
        const { cropId, vehicleId, lat, lng, radiusKm = 100, quantityQuintals, handlingCost = 200, } = req.body;
        ...
        const results = await Promise.all(mandis.map(async (mandi) => {
            const price = await models_1.MarketPrice.findOne({
                mandi: mandi._id,
                crop: crop._id,
            })
                .sort({ date: -1 })
            ...
            const distanceKm = Math.round(haversineKm(Number(lat), Number(lng), mLat, mLng));
