...
router.post("/", async (req, res) => {
    try {
        const trip = await models_1.Trip.create(req.body);
...
router.get("/farmer/:farmerId", async (req, res) => {
    try {
        ...
        const [trips, total] = await Promise.all([
            models_1.Trip.find({ farmer: req.params.farmerId })
                .sort({ tripDate: -1 })
                .skip(skip)
                .limit(limit)
                ...
                .lean(),
            models_1.Trip.countDocuments({ farmer: req.params.farmerId }),
        ]);
        res.json({
            success: true,
            data: trips,
...
router.get("/farmer/:farmerId/summary", async (req, res) => {
    try {
        const summary = await models_1.Trip.aggregate([
...
router.patch("/:id/status", async (req, res) => {
    try {
        ...
        const trip = await models_1.Trip.findByIdAndUpdate(req.params.id, { status }, { new: true }).lean();
        if (!trip) {
